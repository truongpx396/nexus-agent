...
**Scale/Scope**: 175 functional requirements across 9 user stories (see the MVP cut
...
FR-042–FR-045, FR-095–FR-097, FR-117–FR-125, FR-126–FR-130, FR-161, FR-163.
Document conversion (in-sandbox, model-routed) and derived-artifact erasure fold
into the Security & Trust Surface and Delivery/Scale sections above →
FR-160, FR-162.

**Result**: PASS on principles I–IX with **one recorded tension** (see Complexity
Tracking): the constitution's "build for the current stage" rule versus a 175-FR,
...
- The **`PreToolUse`/`PostToolUse` hook layer** (FR-171): the hook dispatcher,
  its three handler types (`command`/`http`/`prompt`), the prompt-hook adjudicator
  hardening (FR-166), and the bounding guards (per-hook timeout, chain budget,
  decision cache) — layer 2 of the permission resolution order, governed and
  eval-gated the same as every other behaviour-bearing config.
...
| Built-in document conversion, retrieval-tier gating, and the scheduled adversarial scan (FR-160, FR-161, FR-163) | The first binary-document ingestion need, the first enabled retrieval tier, and the first scheduled red-team cycle. **The erasure seam is not deferred**: derived-artifact crypto-shredding (FR-162) ships with the Increment 1 erasure path, because a retrieval index or cache built later must already be a shreddable projection of the log |
| Prompt-mode section sets beyond `full` (FR-172) | An operator or surface needs a leaner-than-default prefix. Increment 1 resolves and records `prompt_mode` on every run but ships the `full` section set as the only populated mode. **The seam is not deferred**: `prompt_mode` is a component of `Session.harness_digest` from day one, so adding `task`/`minimal`/`none` later is a new cache prefix rather than a mutation of an existing run, and the configuration-time refusal that stops a mode dropping the safety or tool-contract section ships with the enum — a mode that could relax a control was never representable |
| Progressive memory tiers L1/L2 + the gated memory selector (FR-173) | The file-first `working` tier is exhausted (~1M tokens durable knowledge) — the same trigger as "memory tiers beyond files" above. Increment 1 injects only the resident L0 tier. **The seams are not deferred**: the `memory_loaded` event type is in the Foundational taxonomy and the resolvable memory set is pinned into `Session.harness_digest` from day one, because a run whose memory loads were never recorded as events cannot be replayed and a digest that never pinned the resolvable set cannot be reconstructed later; each L1/L2 index is a `memory_index` derived artifact so it is shreddable the day it is built |
| The adaptation-proposal pipeline (FR-174) | The signal volume justifies mining production for candidate improvements, and the human-review workflow exists to triage them. Increment 1 ships no generator — changes are hand-authored. **The seam is not deferred**: the `AdaptationProposal` record, its `status` projection, and the `eval_run_id`/`governance_signoff`/`rollout_ref` links ship with the schema, because a proposal that could reach `applied` without clearing the gate — or an approved change that shipped outside the FR-140 rollback guardrail — is exactly the metrics→auto-adapt hole FR-044 forbids, and the enforcement point (no proposal is applied without a passing gate and a sign-off, none may widen a capability) is a schema invariant from day one, not a later bolt-on |
| The zero-LLM keyword-recall tool (FR-175) | Bundled with US5's memory work, since it lives in `internal/memory/` alongside the L1/L2 tiers and its practical value compounds once other memory tiers exist to be a cheap fallback for. **This is a sequencing choice, not a technical dependency**: FR-175 explicitly disclaims being a retrieval tier under FR-022 and needs neither the deferred L1/L2 stores nor an enabled retrieval tier to function — it is a standalone BM25 index over the Increment-1 event log. Revisit ahead of US5 if a customer needs session-history search before the rest of memory ships |
