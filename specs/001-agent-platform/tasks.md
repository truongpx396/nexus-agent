...
- [ ] T071b [P] [US3] Route **model-backed conversion** (image description, chart interpretation, transcription, multimodal OCR) through the `Provider` abstraction rather than a converter-owned client — routed by data label, reserved pre-spend, metered per token class, attributed like any other model call — with the capability independently disablable per tenant and defaulting **off** for data labels whose routing floor forbids the converter's configured model. Add an integration test that a `regulated`-labelled document never reaches a model its label forbids, and a CI guard failing the build on a provider SDK imported under `tools/builtin/` (FR-160c, FR-027, FR-037, FR-083, FR-016, SC-064)
