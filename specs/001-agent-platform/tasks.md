...
- [ ] T092g1 [P] [US4] Declare the **`sandbox`-port conformance dimensions** so a `service` executor (OpenSandbox, managed E2B) has an admission path before any adapter exists: attested isolation backend, enforcement of the FR-059 CPU/memory/PID/wall-clock limits on the far side, network default-deny with the FR-037 allowlist decided platform-side, workspace-scoped filesystem view, cold-provisioning for eval trials, region placement for FR-091, and **absence of any in-sandbox route into the tool pipeline that bypasses the FR-149 broker** — a `service` executor with any of these `unsupported` cannot be enabled for a tenant, in `backend-go/internal/integrations/dimensions_sandbox.go` (FR-133, FR-059, FR-047, FR-091, FR-149)
...
- [ ] T107 [US6] Implement deployment-topology configuration (SaaS/single-tenant/BYOC/hybrid) selecting sandbox isolation **and executor** + data-plane placement in `backend-go/internal/tenancy/topology.go` (FR-050, FR-059)
...
- [ ] T118 [P] [US7] Implement the warm sandbox pool (hard TTLs, reclamation on terminal/stuck, per-tenant caps, isolation by topology, gVisor (`runsc`) default backend, placement via the T119c `Executor` port) in `backend-go/internal/sandbox/pool.go` (FR-047, FR-059)
...
- [ ] T119c [P] [US7] Define the **`Executor` port** separating placement from isolation — `local` (default) / `ssh` / `service`, with `executor` + `executor_target` persisted on `Sandbox` — such that every executor returns a sandbox handle carrying its attested `isolation`, resource limits, network policy, and workspace scope, and the pool refuses a handle whose attestation does not satisfy the requested contract rather than admitting it at a weaker boundary, in `backend-go/internal/sandbox/executor/executor.go` (FR-059, FR-047)
- [ ] T119d [P] [US7] Implement the **`ssh` executor** placing sandboxes on a dedicated execution host: credentials drawn from the secrets vault at execution time (never an on-disk key readable at rest), host key pinned and verified, target resolved from configuration and never from model output, remote command construction through the same T119a allowlist, and the target host checked against the tenant's residency pin before admission, in `backend-go/internal/sandbox/executor/ssh.go` (FR-059, FR-034, FR-091)
- [ ] T119e [P] [US7] Integration test — **placement never relaxes the boundary**: an `ssh` executor whose far side reports a weaker `isolation` than requested is refused with a typed reason and no session is admitted; an `ssh` target violating the tenant's residency pin is refused; a remote command carrying an injected argument is rejected by the same allowlist as the local path; and the SSH private key never appears on disk or in any span attribute, in `backend-go/tests/integration/sandbox_executor_ssh_test.go` (FR-059, FR-034, FR-091, FR-117)
