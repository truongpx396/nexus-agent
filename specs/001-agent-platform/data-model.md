...
- **Built-ins**: the platform ships built-in tools — workspace-restricted filesystem (`file_read`/`file_write`/`file_search`/…), a sandboxed shell/code-execution tool, web search/fetch (egress-allowlisted, untrusted results), and a **zero-LLM keyword recall** tool over prior session history that returns raw cited excerpts at no model cost (FR-175) — governed by the same three gates and per-invocation safety (FR-056–FR-058, FR-175).
...
- **The `keyword_index` kind backs the FR-175 zero-LLM keyword recall tier** — a full-text / BM25 projection of the decrypted event log that answers a deterministic recall query over prior session messages at **no model cost** (no embedding, no reranker, so none of the FR-161 drift the model tiers carry). Like every derived artifact it is **deleted, not shredded**, with its source key, and a rebuild after erasure reproduces only the post-erasure history. It is distinct from `vector_index`/`memory_index`, which project the FR-022 model-backed tier.
