...
description: Use when the user wants an Excalidraw diagram for an architecture, workflow, protocol, concept, or visual explanation.
---

# Excalidraw Diagram Skill

Create `.excalidraw` JSON files that explain ideas through structure, not just labeled boxes.

## When to Use

- Architecture diagrams
- Workflow or lifecycle diagrams
- Protocol or event-flow diagrams
- Comparison diagrams
- Visual explanations for specs, plans, or README content

Do not use this skill for plain text summaries that do not benefit from a visual model.

## Outputs

- A `.excalidraw` file that opens in Excalidraw
- When useful, a rendered PNG for validation
- A short note describing what the diagram is meant to communicate

## Design Rules

1. Match the shape of the diagram to the meaning.
2. Prefer a small number of strong visual groups over many uniform cards.
3. Use free-floating labels when a box adds no meaning.
4. Show real names for systems, events, or interfaces when the source material provides them.
5. Keep reading order obvious from top-to-bottom or left-to-right.

## Workflow

### 1. Understand the diagram goal
...
Read `references/layout-patterns.md` and pick the pattern that matches the concept:
...
### 3. Read the visual references

Before generating any diagram, read:

- `references/color-palette.md`
- `references/json-guidelines.md`

Use those files as the source of truth for style and JSON structure.

### 4. Generate the `.excalidraw` JSON

Requirements:

- Use stable, descriptive element ids when hand-authoring JSON
- Use consistent spacing between groups
- Reserve accent colors for emphasis, not for every shape
- Keep text readable at default zoom

### 5. Validate

If the environment supports local setup, render the file with:

```bash
cd .github/skills/excalidraw-diagram/references
uv sync
uv run playwright install chromium
uv run python render_excalidraw.py <input.excalidraw> <output.png>
```
...
- overlapping text
- clipped labels
- arrows crossing important text
- uneven whitespace
- unclear hierarchy

Revise the JSON until the layout is clean.

## Diagram Content Guidance

### Simple conceptual diagrams

Use minimal text and emphasize relationships.

### Technical diagrams

Include concrete evidence where appropriate:

- event names
- API paths
- sample payload snippets
- component names
- storage or queue names

Do not invent technical details that are absent from the source material.

## File Layout

```text
.github/skills/excalidraw-diagram/
├── SKILL.md
├── README.md
└── references/
    ├── color-palette.md
...
- Customize colors in `references/color-palette.md`
- Keep the skill repo-local so the whole team gets the same behavior
- Prefer original diagram structure over mechanically copying an existing chart style---
name: excalidraw-diagram
